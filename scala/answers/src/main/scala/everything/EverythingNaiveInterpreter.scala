...
object EverythingNaiveInterpreter extends Interpreter {
  ...
  def eval(exp: Exp, state: ProgramState): (RuntimeValue,Output,Mem) =
    exp match {
      case Num (i)       => (NumV(i),state.out,state.mem)
      case Add (l,r)     =>
        val (lv,lo,mo1) = eval(l,state)
        val (rv,ro,mo2) = eval(r,ProgramState(state.env,lo,mo1))
      ...
      case Let ((x,e),b) =>
        val (eValue,o1,m1) = eval(e, state)
        eval(b, ProgramState(state.env + (x -> eValue), o1, m1))
      case Print(e)      =>
        val (eValue,o1,m1) = eval(e, state)
      ...
      case Apply(fexp, a) => eval(fexp, state) match {
        case (Closure(func,cEnv),o1,m1) =>
          val (arg,o2,m2) = eval(a, ProgramState(state.env, o1, m1))
          eval(func.body, ProgramState(cEnv + (func.arg -> arg), o2, m2))
      ...
      case SetMem(address: Exp, e:Exp) =>
        val (addrValue,o1,m1) = eval(address, state)
        val (eValue,o2,m2) = eval(e, ProgramState(state.env, o1, m1))
      ...
      case Statements(es)    =>
        es.foldLeft((NumV(0):RuntimeValue,state.out,state.mem)){
          case ((_,outacc, memacc),e) =>
            eval(e, ProgramState(state.env, outacc, memacc))
        }
    }
