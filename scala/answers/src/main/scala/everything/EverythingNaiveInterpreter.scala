...
object EverythingNaiveInterpreter extends Interpreter {
  ...
  case class ProgramException(msg: String, state: PState)
    extends RuntimeException(msg)

  override def interpret(exp: Everything.Exp): (String \/ RuntimeValue, PState) =
      try {
        val t = interp(exp, PState())
        (t._1.right, PState(env = Map(), t._2, t._3))
      }
      catch { case p: ProgramException => (p.msg.left, p.state) }

  def lookup(v: String, state:PState): RuntimeValue =
    state.env.getOrElse(v, err(s"unbound variable, $v", state))

  def readMem(addr: Int, state:PState): RuntimeValue =
    state.mem.getOrElse(addr, err(s"null pointer: $addr", state))

  def interp(exp: Exp, state: PState): (RuntimeValue,Output,Mem) =
    exp match {
      case Num (i)       => (NumV(i),state.out,state.mem)
      case Add (l,r)     =>
        val (lv,lo,mo1) = interp(l,state)
        val (rv,ro,mo2) = interp(r,PState(state.env,lo,mo1))
      ...
      case Let ((x,e),b) =>
        ...
        interp(b, PState(state.env + (x -> eValue), o1, m1))
      case Print(e)      =>
        val (eValue,o1,m1) = interp(e, state)
      ...
      case Apply(fexp, a) => interp(fexp, state) match {
        case (Closure(func,cEnv),o1,m1) =>
          val (arg,o2,m2) = interp(a, PState(state.env, o1, m1))
          interp(func.body, PState(cEnv + (func.arg -> arg), o2, m2))
      ...
      case SetMem(address: Exp, e:Exp) =>
        ...
        val (eValue,o2,m2) = interp(e, PState(state.env, o1, m1))
      ...
      case GetMem(address: Exp) =>
        val (addrValue,o1,m1) = interp(address, state)
        addrValue match {
          case NumV(addr) => (readMem(addr, PState(state.env, o1, m1)), o1, m1)
          case c => err(s"invalid memory address: $c", state)
        }
      case Statements(es)    =>
        es.foldLeft((NumV(0):RuntimeValue,state.out,state.mem)){
          case ((_,outacc, memacc),e) => interp(e, PState(state.env, outacc, memacc))
        }
    }

  def math(l:RuntimeValue, r:RuntimeValue, s:PState)
          (f: (Int, Int) => Int): RuntimeValue = (l,r) match {
    case (NumV(lv), (NumV(rv))) => NumV(f(lv,rv))
    case bad => err(s"can't add: $bad", s)
  }

  def err(msg: String, s: PState) = throw new ProgramException(s"$msg", s)
}
