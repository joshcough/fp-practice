...
//  test(("x" -> print(9.n) in print(v"x")) mustBe (List(9,9), Map(), 9))
  ...
  test(("x" -> print(9.n) in print(8.n)) mustBe (List(9,8), Map(), 8))
...
trait Helpers { self: Properties =>
  ...
  def testBody(t: (Exp,Output,Mem,RuntimeValue))
    ...
    secure {
      val v: (String \/ RuntimeValue, ProgramState) = i.interpret(t._1)
      println(s"\ngot $v, expected ${t._4.right -> ProgramState(Map(), t._2, t._3)}\n")
      Prop.all(
        //check(v._1, t._4.right, "result must be correct"),
        (v._1 == t._4.right) :| s"result must be correct ${v._1 -> t._4.right}",
        (v._2.out == t._2)   :| s"output must be correct ${v._2.out -> t._2}",
        (v._2.mem == t._3)   :| s"memory must be correct ${v._2.mem -> t._3}"
      )
    }

  def check[A:Equal](l:A, r:A, msg:String) =
    (l === r) :| s"$msg (left: $l, right: $r)"
